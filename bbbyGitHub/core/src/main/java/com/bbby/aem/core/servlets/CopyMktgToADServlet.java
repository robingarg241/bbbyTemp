...
@Component(
        name = "Marketing AD Copy Servlet",
        immediate = true,
        service = Servlet.class,
        property = {
                "sling.servlet.methods=POST",
                "sling.servlet.paths=/bin/bbby/copy-mktg"
        }
)
public class CopyMktgToADServlet extends SlingAllMethodsServlet {
    ...
    protected void doPost(final SlingHttpServletRequest req, final SlingHttpServletResponse resp) throws ServletException, IOException {
     ...
     private String finalAssetNameFolderCreation(String assetPath, Session session) throws Exception {
		...
		String newName = "m-" +ZonedDateTime.now().toString().replaceAll("[^0-9-]", "-").substring(0,16)+"_"+assetName;
         ...
         if(!session.nodeExists(dest)) {
             ...
             if(session.nodeExists(dest+"/jcr:content/metadata")) {
                 ...
                 metadataNode.setProperty(CommonConstants.BBBY_UPLOADED_ASSET_NAME, assetName);
                 	...
                 	String[] tags ={"bbby:asset_type/approved_dam/assets/product/images/single_product","bbby:shot_type/silhouette"};
