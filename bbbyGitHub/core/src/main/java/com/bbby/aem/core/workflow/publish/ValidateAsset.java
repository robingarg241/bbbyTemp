 ...
 *
 */
@Component(service = WorkflowProcess.class, property = { "process.label=Publish Validate Asset",
...
public class ValidateAsset implements WorkflowProcess {

	protected final Logger log = LoggerFactory.getLogger(this.getClass());

	private ResourceResolver resourceResolver;

	private static String workflowName = "/var/workflow/models/bbby-approve-and-publish--beta-";
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap metaDataMap)
		...
		if (CommonConstants.JCR_PATH.equalsIgnoreCase(type)) {
				...
				if (session.itemExists(path) && !path.contains("content/dam/bbby/approved_dam")) {
					...
					if(isValidAssetName) {
						if (!checkSameWorkflowNotRunning(path, workItem.getWorkflow().getId())) {
							...
							if (S7SetHelper.isS7Set(image1r)) {
								...
								if (hasReferences) {
									...
									if (checkAssetsSyncToS7(assets)) {
										...
										log.info(
												"Unable to start Project CleanUp Workflow, One or more assets are not sync with Scene7. Please Check Operational Attribute of added Assets.");
									...
									log.info(
											"Unable to start Project CleanUp Workflow, One or more assets are not have reference. Please Check Operational Attribute of added Assets.");
							...
							workflowData.getMetaDataMap().put("isAlreadyRunning", "true");
			...
			if (isValidAssetName && allAssetNameValid) {		
				if("true".equalsIgnoreCase(isAlreadyRunning) || "false".equalsIgnoreCase(isSyncToS7) || "false".equalsIgnoreCase(hasReferences)){
					workflowData.getMetaDataMap().put("checkReferenceStep", "false");
				}else if("false".equalsIgnoreCase(isAlreadyRunning) && "true".equalsIgnoreCase(isSyncToS7) && "true".equalsIgnoreCase(hasReferences)){
	...
	private boolean checkSameWorkflowNotRunning(String path, String workflowId) throws Exception {
		...
		if (workflows != null && workflows.size() > 0) {
			for (Workflow workflow : workflows) {
				if (workflow.getWorkflowModel().getId().equalsIgnoreCase(workflowName)
						&& !workflow.getId().equalsIgnoreCase(workflowId)
						&& workflow.getState().equalsIgnoreCase("RUNNING")) {
					isAlreadyRunning = true;
					log.info("Already Running BBBY Project CleanUp Workflow with Id {} on path {}", workflow.getId(),
							path);
				}
			}
		}
