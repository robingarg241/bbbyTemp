 ...
 *
 */
@Component(service = WorkflowProcess.class, property = { "process.label=Publish Assets",
		"Constants.SERVICE_VENDOR=BBBY",
		"Constants.SERVICE_DESCRIPTION=BBBY APPROVE AND PUBLISH, Publish Imageset/Image" })
public class PublishAssetorImageset implements WorkflowProcess {
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap metaDataMap)
		...
		try {
			...
			String destination = getImagesetDestPath(path.substring(path.lastIndexOf("/")+1, path.length()));
			...
			if (S7SetHelper.isS7Set(image1r)) {
				...
				if (assets != null) {
					...
					for (Node memberNode : assets) {
						...
						Node repometa = JcrPropertiesUtil.getReportingNode(memberNode, session);
						if (repometa != null) {
