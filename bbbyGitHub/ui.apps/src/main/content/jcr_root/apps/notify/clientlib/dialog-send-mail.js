(function ($, $document) {
    var BUTTON_URL_NEW = "/apps/notify/content/send-mail-but.html",
    	NOTIFY_ACTIVATOR= "cq-damadmin-admin-actions-send-mail-activator",
        SHARE_ACTIVATOR = "cq-damadmin-admin-actions-adhocassetshare-activator",
        SEND_MAIL_SERVLET = "/bin/bbby/notify",
        SEND_MAIL_URL = "/apps/notify/send-mail-dialog.html",
    ...
    function getModalIFrameUrl(subject, body, assets){
        var url = Granite.HTTP.externalize(SEND_MAIL_URL) + "?" + REQUESTER + "=" + SENDER;

        url = url + "&subject=" + encodeURIComponent(subject) + "&body=" + encodeURIComponent(body) + "&assets=" + encodeURIComponent(assets);
