...
String resourceParentPath = resourcePath.substring(0,resourcePath.lastIndexOf('/') + 1);
%>
<coral-quickactions scrollonfocus="off" target="_prev" alignmy="left top" alignat="left top">
    ...
    <%   if (isAdmin || (canEdit && !isAssetExpired && !isSubAssetExpired && canUseRestricedActions)) {
    ...
    <% } %>
        ...
        <% } %>
    <coral-quickactions-item icon="open" class="foundation-anchor" data-foundation-anchor-href="<%= xssAPI.getValidHref(request.getContextPath() + "/assets.html" + resourceParentPath)%>" data-pageheading="<%= i18n.get("AEM Assets | Asset Editor")%>"><%= xssAPI.encodeForHTML(i18n.get("Folder")) %></coral-quickactions-item>
